---
permalink: /
title: "তাশফিয়া সাদ | Tashfia Saad"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---
I am interested in Machine Learning and mathematically grounded approaches to Artificial Intelligence. I am currently exploring bioinformatics, with a focus on machine learning-driven biological data analysis and computational methods for understanding complex biological systems. I am pursuing an MSc in Computer Science and Engineering at BRAC University, where I am also working as a Graduate Teaching Assistant. Additionally, I am a part-time Lab Instructor at North South University.

## Research Interests
- Machine Learning
- Artificial Intelligence
- Bioinformatics
- Computational Biology
- Material Science
- Natural Language Processing
- Computer Vision
- Generative AI

## Education
- **M.Sc. Computer Science and Engineering** *(Ongoing)*, BRAC University, 2026
- **B.Sc. Computer Science and Engineering**, BRAC University, 2025 — CGPA 3.83/4.00, Highest Distinction

## Ongoing Research

**Reproducing ML-based Compound Stability Predictions** 

I am currently reproducing the findings of Bartel et al. (npj Computational Materials, 2020), which critically examines whether ML models that accurately predict formation energy (ΔHf) can also reliably predict thermodynamic stability (ΔHd). My reproduction involves implementing all 6 compositional models (Roost, ElemNet, Meredig, Magpie, AutoMat, ElFrac) on ~154k Materials Project compounds, with a focus on convex hull construction for stability classification.

## Contact
Feel free to reach out at [tashfia.saad@bracu.ac.bd](mailto:tashfia.saad@bracu.ac.bd)
